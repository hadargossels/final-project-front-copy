...
export default class Product extends Component {
   ...
   constructor(props) {
      ...
      if (product.length === 1) {
         ...
         this.state = {arr: product[0], src: product[0].img[0], type: product[0].type, addNotify: "", status: "", price: "", newPrice: "", quantity: 1, addProductCart: this.props.addProductCart};
         ...
         if (this.state.arr.stock) {
            this.state.addNotify = <button type="button" className="btn btn-outline-primary" onClick={() => this.state.addProductCart(this.state.arr.id, this.state.quantity)}><span style={{fontSize: "1.25rem", fontWeight: "300"}}>Add to <i className="fas fa-shopping-cart"></i></span></button>;
