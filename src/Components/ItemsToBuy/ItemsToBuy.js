...
class ItemsToBuy extends Component {
    ...
    removeItem = (event) => {
        ...
        let newStorageList = storageList.filter((item) => {return item !== itemId})
        let productsList = this.state.productList
        let newList = productsList.filter((product) => {return product.ISBN10 !== itemId})
      ...
      return (
        <div className="itemsToBuy m-5">
            ...
            {this.state.productList.map((product) => {
                return (
                    <div className="item bg-gray-300 text-xl pt-5 pb-1 pl-5 grid grid-cols-5 grid-rows-1 rounded mb-10 border-4 border-yellow-700" key={product.ISBN13}>
                        <div className="col-span-2 row-span-1">
                            <img src={product.image} alt="" className="cartImage float-left mr-5"/>
                        ...
                        <div className="col-span-1 pt-12 text-center">
                            <input type="number" name="quantity" min="1" max="10" defaultValue="1" className="quantity pl-2 pt-1 border-0" id={product.ISBN10} 
                        ...
                        <div className="col-span-1 text-3xl pt-12 ml-5 text-center">
                            <button id={product.ISBN10} className="bg-yellow-800 text-yellow-100 pt-3 px-4 py-2 shadow-lg border border-yellow-800 hover:bg-yellow-100 hover:text-yellow-800 hover:shadow-none focus:shadow-none rounded"
                            onClick={(event) => this.removeItem(event)}>
                                <i className="fas fa-trash-alt" id={product.ISBN10}></i>
                            </button>
                        </div>
                    </div>
                )
